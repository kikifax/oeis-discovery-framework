...
class RaylibExplorer
  ...
  def pump_generation
    ...
    auto_fit_all(@target_terms) if @terms.size % 100 == 0
  end
  ...
  def draw
    ...
    if @sidebar_tab == :catalog
      ...
      panel_y = 120
      draw_text_safe("ANALYTICS", 40, panel_y, 16, @accent)
      if @analysis
        draw_text_safe("Growth: #{@analysis[:stats][:growth_type]}", 40, panel_y + 50, 18, @color_black)
        draw_text_safe("Score: #{@analysis[:fitness_score]}/100", 40, panel_y + 85, 18, @color_black)
      end
    end
